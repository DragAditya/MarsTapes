...
import { Plus, Trash2, RotateCcw, Save, List, Calendar } from 'lucide-react';
...
export function ReceiptForm() {
  ...
  // BUG FIX: Renamed from "Share" to "Save" — the action saves the receipt and
  // advances to a new one. This now correctly reflects what the button does.
  ...
  const handleReset = useCallback(() => {
    if (general.confirmBeforeDeleting && (currentReceipt.customerName || currentReceipt.lineItems.length > 0)) {
    ...
    <div className="w-full max-w-3xl mx-auto pb-8">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 sm:p-6">
        <div className="flex items-center gap-2 mb-5">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: general.primaryColor + '12' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1.5">Receipt No. (Auto)</label>
            <input
              type="text"
          ...
          <div>
            ...
            <div className="relative">
              <input
                type="date"
                // BUG FIX: date is stored as DD/MM/YYYY internally. Convert to YYYY-MM-DD for the input.
                // The previous code used split('/').reverse() which only works for DD/MM/YYYY.
                // Now we parse it properly regardless of storage format.
                value={(() => {
                  ...
                  return currentReceipt.date; // fallback for ISO
                })()}
                onChange={e => {
                  // BUG FIX: Parse the ISO date string directly — no new Date() to avoid timezone shift.
                  // new Date("2025-05-24") treats it as UTC midnight, which shifts the date in some timezones.
                  const [year, month, day] = e.target.value.split('-');
                  if (year && month && day) {
                    dispatch({ type: 'SET_CURRENT_RECEIPT', payload: { date: `${day}/${month}/${year}` } });
                  }
                }}
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1.5">M/s. (Customer Name)</label>
          <input
            type="text"
            placeholder="Customer / Party name"
            value={currentReceipt.customerName}
            onChange={e => dispatch({ type: 'SET_CURRENT_RECEIPT', payload: { customerName: e.target.value } })}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 sm:p-6 mt-4">
        ...
        <div className="mt-4 pt-4 border-t border-gray-100">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-sm font-mono text-gray-900">{currentReceipt.subtotal.toFixed(2)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-base font-semibold text-gray-900">Grand Total ({symbol})</span>
            <span className="text-lg font-bold font-mono" style={{ color: general.primaryColor }}>
      ...
      {/* BUG FIX: Removed misleading "Automatically saved to Google Drive" label.
          Google Drive sync is not implemented. Data is only saved to localStorage. */}
...
function LineItemRow({ item, index }: { item: { id: string; particular: string; qty: number; unit: string; rate: number; amount: number }; index: number }) {
  const { state, dispatch } = useApp();
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-[40px_1fr_80px_80px_100px_100px_44px] gap-2 p-2 bg-gray-50/50 rounded-lg border border-gray-100">
      <span className="hidden sm:flex items-center text-xs font-mono text-gray-500">{index + 1}</span>

      <div className="col-span-1 sm:col-span-1">
        <label className="sm:hidden text-[10px] font-medium text-gray-500 uppercase">Particulars</label>
        <input
          type="text"
          list="particulars-list"
          value={item.particular}
          onChange={e => dispatch({ type: 'UPDATE_LINE_ITEM', payload: { id: item.id, field: 'particular', value: e.target.value } })}
      ...
      <div>
        <label className="sm:hidden text-[10px] font-medium text-gray-500 uppercase">Qty</label>
        <input
          type="number"
          min="0"
          step="1"
          ...
          onChange={e => {
            ...
            dispatch({ type: 'UPDATE_LINE_ITEM', payload: { id: item.id, field: 'qty', value: val } });
          }}
      ...
      <div>
        <label className="sm:hidden text-[10px] font-medium text-gray-500 uppercase">Unit</label>
        <select
          value={item.unit}
          onChange={e => dispatch({ type: 'UPDATE_LINE_ITEM', payload: { id: item.id, field: 'unit', value: e.target.value } })}
      ...
      <div>
        <label className="sm:hidden text-[10px] font-medium text-gray-500 uppercase">Rate</label>
        <input
          type="number"
          min="0"
          step="0.01"
          ...
          onChange={e => {
            const val = e.target.value === '' ? 0 : Number(e.target.value);
            dispatch({ type: 'UPDATE_LINE_ITEM', payload: { id: item.id, field: 'rate', value: val } });
          }}
      ...
      <div>
        <label className="sm:hidden text-[10px] font-medium text-gray-500 uppercase">Amount</label>
